...
public class TeacherController {
    ...
    public String hello(Model msg) {
        ...
        msg.addAttribute("teacherName", teacher.getTeacherName());
        return "/teacher/main";
    }
    ...
    public String showTheCourseTaught(Model msg) {
        try {
            ...
            Set<Course> allTaughtCourses = teacher.getCourses();
            List<List<String>> response = new ArrayList<>();
            for (var i : allTaughtCourses) {
    ...
    public String showTheCourseStudents(Model msg) {
        try {
            String username = getCurrentTeacherUserName();
            Teacher teacher = teacherService.getTeacherByUsername(username);
            List<Course> allTaughtCourses = courseService.findAllCoursesByTeacherId(teacher.getTeacherId());
            ...
            for (var i : allTaughtCourses) {
                //添加课程信息
                List<String> course = new ArrayList<>();
                course.add(i.getSubject().getSubjectName()); //courseName 0
                course.add(i.getSubject().getSubjectId().toString()); //subjectId 1
                course.add(i.getClassNumber().toString()); //classNumber 2
                course.add(courseService.getAverageScore(i).toString());//averageScore 3
                course.add(courseService.getPassRate(i).toString()); //passRate 4
                course.add(courseService.getExcellentRate(i).toString());//ExcellentRate 5
